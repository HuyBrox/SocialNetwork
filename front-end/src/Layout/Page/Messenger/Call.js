...
export function Call({ selectedChat }) {
  ...
  const [isVideoOn, setIsVideoOn] = useState(false); // Video bật/tắt
  ...
  const setupMediaStream = async (callType) => {
    try {
      ...
      console.log('Stream trả về:', stream);
      ...
      if (localVideoRef.current) {
        ...
        localVideoRef.current.muted = true; // Tắt tiếng local
      }
  ...
  const toggleMic = () => {
    if (localStreamRef.current) {
      ...
      console.log('Audio tracks trước khi toggle:', audioTracks);
      if (audioTracks.length > 0) {
        const enabled = !isMicOn;
        audioTracks.forEach((track) => (track.enabled = enabled)); // Thay đổi trạng thái track
        console.log('Trạng thái audio sau toggle:', audioTracks.map(track => track.enabled));
        setIsMicOn(enabled); // Cập nhật trạng thái mic
      } else {
        console.warn('Không tìm thấy track audio để tắt/mở mic.');
      }
    }
  };
  ...
  const toggleVideo = () => {
    if (localStreamRef.current) {
      ...
      if (videoTracks.length > 0) {
        ...
        console.warn('Không tìm thấy track video. Yêu cầu lại media stream.');
        // Yêu cầu lại media stream nếu cần thiết
