...
const MessengerLayout = () => {
  ...
  const fetchUserStatuses = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/message/person', {
        method: 'GET',
        credentials: 'include'
      });
  ...
  const fetchUserInfo = async (userId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/message/getMess/${userId}`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const searchUsers = async (keyword) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/message/Search?keyword=${encodeURIComponent(keyword)}`, // Sửa từ messages thành message
        {
          method: 'GET',
          credentials: 'include'
        }
