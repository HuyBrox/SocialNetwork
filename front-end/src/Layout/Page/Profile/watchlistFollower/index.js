...
const FollowerModal = ({
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const response = await fetch(`https://s1-api.vercel.app/api/user/followers/${youUserId || myUserId}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      if (data.success) {
        const myFollowingResponse = await fetch(`https://s1-api.vercel.app/api/user/following/${myUserId}`, {
          method: 'GET',
          credentials: 'include',
        });
        ...
        for (const user of data.followers) {
          const userFollowingResponse = await fetch(`https://s1-api.vercel.app/api/user/following/${user._id}`, {
            method: 'GET',
            credentials: 'include',
          });
  ...
  const handleConfirmUnfollow = async () => {
    ...
    try {
      const response = await fetch(`https://s1-api.vercel.app/api/user/follower/${selectedUserId}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
