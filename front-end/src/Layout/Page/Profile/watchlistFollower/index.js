...
const FollowerModal = ({
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/user/followers/${youUserId || myUserId}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      if (data.success) {
        const myFollowingResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/user/following/${myUserId}`, {
          method: 'GET',
          credentials: 'include',
        });
        ...
        for (const user of data.followers) {
          const userFollowingResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/user/following/${user._id}`, {
            method: 'GET',
            credentials: 'include',
          });
  ...
  const handleConfirmUnfollow = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/user/follower/${selectedUserId}`, {
        method: 'DELETE',
        credentials: 'include',
        headers: {
