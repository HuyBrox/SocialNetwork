...
const FollowingModal = ({
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch(`https://s1-api.vercel.app/api/user/following/${targetUserId || currentUserId}`, {
          method: 'GET',
          credentials: 'include',
        });
        ...
        if (data.success) {
          const currentUserFollowingResponse = await fetch(`https://s1-api.vercel.app/api/user/following/${currentUserId}`, {
            method: 'GET',
            credentials: 'include',
          });
