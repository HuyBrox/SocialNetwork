...
const Profile = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (!hasFetchedProfile.current && (userId || youUserId !== myUserId)) {
          const response = await fetch(`http://localhost:5000/api/user/${youUserId}/profile`, {
            method: 'GET',
            credentials: 'include',
          });
  ...
  const handleFollowToggle = async (targetUserId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/user/followorunfollow/${targetUserId}`,
        {
          method: 'POST',
          credentials: 'include',
        }
  ...
  const handleRemoveProfilePicture = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/user/deleteAvatar`, {
        method: 'DELETE',
        credentials: 'include',
      });
      ...
      const data = await response.json();
      if (data.success) {
        const profileResponse = await fetch(`http://localhost:5000/api/user/${userId}/profile`, {
          method: 'GET',
          credentials: 'include',
        });
  ...
  const handleUpdateFeaturedNote = async (newFeaturedNote) => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/user/featuredNote', {
        method: 'POST',
        headers: {
    ...
    <div className="profile">
      ...
      {showPreview && (
        <PreviewNote
          ...
          onDelete={async () => {
            try {
              await fetch('http://localhost:5000/api/user/featuredNote', {
                method: 'DELETE',
                headers: {
                  'Content-Type': 'application/json',
                },
