...
export const fetchPosts = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const response = await fetch(`http://localhost:5000/api/post/getUserPost/${userId}`, {
        method: 'GET',
        credentials: 'include',
      });
  ...
  async ({ postId, sortType = 'intelligent' }, { rejectWithValue }) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/comment/${postId}?sortType=${sortType}`,
        {
          method: 'GET',
          credentials: 'include',
        }
...
export const replyComment = createAsyncThunk(
  ...
  async ({ postId, commentId, text }, { rejectWithValue }) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/comment/reply/${postId}/${commentId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
